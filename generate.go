...
{{/**/}}//go:build !go1.20 && ({{range $i, $tag := .Tags}}{{if gt $i 0}} || {{end}}{{$tag}}{{end}}) && !generate && !endian_nostatic
{{/**/}}// +build !go1.20
...
// Native is the byte order of GOARCH.
// It will be determined at runtime because that GOARCH was unknown at code
// generation time.
var Native nativeEndian

// See nativeEndian as defined in Go 1.20+
type nativeEndian struct {
	byteOrder // private embedding
}

type byteOrder = {{- if eq .Go "1.19"}}interface {
	binary.ByteOrder
